...
export class LoginFormularioComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.usuarioService.devolveUsuarios().subscribe((usuarios: IUsuario[]) => {this.listaUsuarios = Buffer.from(JSON.stringify(usuarios))});
  ...
  public validaUsuario(){
    console.log(this.listaUsuarios.toString());
